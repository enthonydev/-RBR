...
function Simulation({ step, setStep, setScreen, onGenerate }: { step: SimulationStep; setStep: (step: SimulationStep) => void; setScreen: (screen: Screen) => void; onGenerate: (simulation: SimulationConfig) => void }) {
...
function AmortizationTable({ comparison, setScreen }: { comparison: SimulationComparison; setScreen: (screen: Screen) => void }) {
  const [search, setSearch] = useState("");
  const [showAdd, setShowAdd] = useState(false);
  const rows = comparison.scenario.schedule.slice(0, 10).map((row) => ({ ...row, monthLabel: formatMonth(row.month) }));
  const filtered = rows.filter((row) => `${row.monthLabel} ${row.month}`.toLowerCase().includes(search.toLowerCase()));
  return <section className="table-page"><section className="page-intro"><div><div className="eyebrow"><span className="eyebrow-line" />DETALHAMENTO</div><h1>Tabela de amortização</h1><p>Consulte a evolução do contrato, mês a mês.</p></div><button className="primary-button" onClick={() => setShowAdd(true)}><Plus size={16} /> Adicionar amortização</button></section><div className="table-toolbar"><div className="table-tabs"><button className="active">Amortização mensal</button><button onClick={() => setScreen("result")}>Resumo da estratégia</button></div><div className="table-tools"><div className="search-input"><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar mês" /></div><button className="filter-button"><CalendarDays size={15} /> {formatMonth(1)} — {formatMonth(comparison.scenario.payoffMonth)} <ChevronDown size={14} /></button></div></div><div className="panel table-panel"><div className="table-caption"><div><span className="section-kicker">CRONOGRAMA CALCULADO</span><h2>Primeiros movimentos</h2></div><StatusPill tone="accent">Price · {comparison.scenario.payoffMonth} meses</StatusPill></div><div className="data-table-wrap"><table><thead><tr><th>MÊS</th><th>PARCELA</th><th>JUROS</th><th>AMORTIZAÇÃO</th><th>SALDO DEVEDOR</th><th /></tr></thead><tbody>{filtered.map((row) => <tr key={row.month} className={row.extraordinary > 0 || row.month === 1 ? "highlight-row" : ""}><td><div className="month-cell"><span className="month-number">{String(row.month).padStart(2, "0")}</span><strong>{row.monthLabel}</strong>{row.month === 1 && <span className="row-tag now">Agora</span>}{row.extraordinary > 0 && <span className="row-tag extra">Extraordinária</span>}</div></td><td>{formatCurrency(row.payment)}</td><td className="muted-value">{formatCurrency(row.interest)}</td><td className={row.extraordinary > 0 ? "green-value" : "muted-value"}>{formatCurrency(row.principal + row.extraordinary)}</td><td><strong>{formatCurrency(row.closingBalance)}</strong></td><td><button className="row-more" aria-label={`Mais opções para ${row.monthLabel}`}><MoreHorizontal size={17} /></button></td></tr>)}</tbody></table></div><div className="table-footer"><span>Mostrando {filtered.length} de {comparison.scenario.schedule.length} parcelas</span></div></div><div className="table-insight"><div className="insight-icon"><Info size={17} /></div><p><strong>Amortização extraordinária.</strong> O aporte é abatido do saldo na parcela informada.</p><button className="text-button" onClick={() => setScreen("result")}>Voltar ao resultado <ArrowRight size={14} /></button></div>{showAdd && <div className="modal-backdrop" onClick={() => setShowAdd(false)}><div className="modal-card" onClick={(event) => event.stopPropagation()}><div className="modal-heading"><div><span className="section-kicker">NOVA ENTRADA</span><h2>Adicionar amortização</h2></div><IconButton label="Fechar" onClick={() => setShowAdd(false)}><X size={17} /></IconButton></div><p>Novos aportes serão incluídos quando a persistência do contrato estiver disponível.</p><button className="primary-button full" onClick={() => setShowAdd(false)}>Fechar</button></div></div>}</section>;
}
...
export default function Home() {
  ...
  const generateSimulation = (next: SimulationConfig) => {
    setSimulation(next);
    setScreen("result");
  };

  return <AppShell screen={screen} setScreen={goTo} onToast={notify}>{screen === "dashboard" && <Dashboard setScreen={goTo} onToast={notify} />}{screen === "simulation" && <Simulation step={simulationStep} setStep={setSimulationStep} setScreen={goTo} onGenerate={generateSimulation} />}{screen === "result" && <Result comparison={comparison} setScreen={goTo} />}{screen === "table" && <AmortizationTable comparison={comparison} setScreen={goTo} />}{screen === "scenarios" && <Scenarios comparison={comparison} simulation={simulation} setScreen={goTo} />}{toast && <div className="toast"><div className="toast-check"><Check size={14} /></div><span>{toast}</span><button onClick={() => setToast(null)} aria-label="Fechar mensagem"><X size={14} /></button></div>}</AppShell>;
}
