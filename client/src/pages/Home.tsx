...
function DebtChart({ simulation }: { simulation: SimulationComparison["baseline"] }) {
  const source = simulation.schedule;
  const step = Math.max(1, Math.floor((source.length - 1) / 5));
  const points = Array.from({ length: 6 }, (_, index) => source[Math.min(index * step, source.length - 1)]).filter(Boolean);
  const maximum = Math.max(simulation.schedule[0]?.openingBalance ?? 0, ...points.map((point) => point.closingBalance), 1);
  const coords = points.map((point, index) => ({ x: points.length === 1 ? 0 : index * 780 / (points.length - 1), y: 24 + (1 - point.closingBalance / maximum) * 216 }));
  const line = coords.map((point, index) => `${index === 0 ? "M" : "L"}${point.x},${point.y}`).join(" ");
  const area = `${line} L780,255 L0,255 Z`;
  return <div className="debt-chart"><div className="chart-y-labels">{[1, .75, .5, .25, 0].map((ratio) => <span key={ratio}>{formatCurrency(maximum * ratio)}</span>)}</div><svg viewBox="0 0 780 255" preserveAspectRatio="none" role="img" aria-label="Evolução projetada do saldo devedor">{[25, 79, 133, 187, 241].map((y) => <line key={y} x1="0" x2="780" y1={y} y2={y} stroke="#e6e8e3" strokeWidth="1" strokeDasharray="3 5" />)}<path d={area} fill="#e7edf5" /><path d={line} fill="none" stroke="#2457a6" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />{coords.map((point) => <circle key={`${point.x}-${point.y}`} cx={point.x} cy={point.y} r="4" fill="#fff" stroke="#2457a6" strokeWidth="2.5" />)}</svg><div className="chart-x-labels">{points.map((point) => <span key={point.month}>{point.month === 1 ? "Hoje" : formatMonth(point.month)}</span>)}</div></div>;
}
...
function Dashboard({ setScreen, onToast, comparison }: { setScreen: (screen: Screen) => void; onToast: (text: string) => void; comparison: SimulationComparison }) {
  return <>
...
function Simulation({ step, setStep, setScreen, onGenerate }: { step: SimulationStep; setStep: (step: SimulationStep) => void; setScreen: (screen: Screen) => void; onGenerate: (simulation: SimulationConfig) => void | Promise<void> }) {
  ...
  const [method, setMethod] = useState<FinancingInput["method"]>("price");
