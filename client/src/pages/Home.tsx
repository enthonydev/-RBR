...
import { parseCurrency, simulateFinancing, type ExtraordinaryPayment, type FinancingInput } from "@shared/finance";
...
const defaultFinancing: FinancingInput = {
  principal: 270000,
  annualRate: 12.5,
  termMonths: 360,
  method: "price",
...
const defaultExtraPayments: ExtraordinaryPayment[] = [{ month: 10, amount: 20000 }];
...
const scheduleRows = defaultProjection.schedule.slice(0, 10).map((row) => ({
  month: new Date(2026, row.month - 1, 1).toLocaleDateString("pt-BR", { month: "short", year: "2-digit" }).replace(".", ""),
